...
{ NSString *string;
  ...
  static char inited = 0;
	if( !inited ){
		LoggerSetOptions( NULL, kLoggerOption_BrowseBonjour|kLoggerOption_CaptureSystemConsole );
		inited = 1;
	}
	if( doLog ){
	  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
		...
		ret = [string length];
		LogMessageF( fileName, lineNumber, functionName, @"QTImage2Mov", level, string );
		[string release];
		[pool drain];
	}
	return ret;
