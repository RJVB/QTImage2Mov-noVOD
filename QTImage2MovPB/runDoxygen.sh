...
if ! [ -f $ROOT/doxygen.config ] 
then 
  echo doxygen config file does not exist
  $DOXYGEN_PATH -g $ROOT/doxygen.config
fi
...
cp $ROOT/doxygen.config $TEMP_DIR/doxygen.config

echo "INPUT = $ROOT ${ROOT}/../brigade" >> $TEMP_DIR/doxygen.config

OUTPUT_DIRECTORY="$ROOT/QTImage2Mov.docset"
echo "OUTPUT_DIRECTORY = $OUTPUT_DIRECTORY" >> $TEMP_DIR/doxygen.config
# set in doxygen.config
# echo "DOCSET_BUNDLE_ID       = fr.INRETS.QTImage2Mov" >> $TEMP_DIR/doxygen.config
